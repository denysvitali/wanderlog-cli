...
var testTripID = func() string {
	if v := os.Getenv("WANDERLOG_TEST_TRIP_ID"); v != "" {
		return v
	}
	return "test-trip-id"
}()
...
func TestMCPIntegration_ContextWithTripID(t *testing.T) {
	...
	// Test with trip ID
	const placeholderID = "context-trip-id"
	ctxWithTrip := withTripID(ctx, placeholderID)
	tripID, ok = tripIDFromContext(ctxWithTrip)
	assert.True(t, ok)
	assert.Equal(t, placeholderID, tripID)
}
